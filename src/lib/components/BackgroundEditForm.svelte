...
<FormSection title="Vorteile">
  {#each background.benefits as benefit, i}
    <FeatureRow bind:nameDe={benefit.nameDe} bind:name={benefit.name}
      namePlaceholder="Vorteil (DE)" removeTitle="Vorteil entfernen"
      onchange={mark} onremove={() => removeBenefit(i)}>
      {#snippet lead()}
